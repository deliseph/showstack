...
${S('The other half', 'Colour psychology, and which parts of it survive',
  ['Colour is the fastest emotional lever on a stage, and it is also the subject of more confident nonsense than anything else in this trade. So it is worth separating what is physiology from what is culture from what is marketing.',
   '<strong>What is genuinely physiological.</strong> The eye has a fifth photoreceptor beyond the three cones and the rods &mdash; intrinsically photosensitive retinal ganglion cells, most sensitive around 480&nbsp;nm, in the blue. They do not contribute much to what you see; they feed the circadian system and measurably affect alertness. Short-wavelength-heavy light in the evening delays sleep and raises alertness, and warm low-level light does the opposite. That is a real, replicated effect with a known mechanism, and it is why a get-out under 5600&nbsp;K feels different from one under 2700&nbsp;K in a way that is not imagination.',
   'Two more that are solidly physiological. <strong>Saturated deep red flickers differently</strong> and is treated separately in the photosensitivity guidance for that reason, which <a href="/learn/access/">the access page</a> covers. And <strong>the eye cannot focus red and blue at the same distance</strong> &mdash; the lens is dispersive, so a saturated red and a saturated blue at the same depth resolve at slightly different focal planes. That is chromostereopsis, it is why deep blue text on a red ground appears to float, and it is a genuine optical effect rather than a perceptual quirk.',
   '<strong>What is learned rather than innate.</strong> Nearly all specific hue-to-meaning mappings. White is mourning in parts of Asia and celebration in parts of Europe. Red is danger on a control panel, luck at a Chinese wedding, and left-wing in one political vocabulary and right-wing in another. These associations are strong, they are real, and they are <em>local</em> &mdash; a lighting choice that reads as ceremonial in one venue reads as funereal in another, and no amount of theory tells you which without knowing the room.',
   '<strong>What does not survive contact with the evidence.</strong> The popular hue-to-emotion tables. &ldquo;Blue is calming, yellow is happy, green is balanced&rdquo; is a marketing artefact rather than a finding: the effects are small, inconsistent between studies, heavily dependent on saturation and brightness rather than hue, and confounded by exactly the cultural associations above. This does not mean colour has no emotional effect &mdash; it plainly does &mdash; it means the effect is not carried by hue alone in the way the tables claim.'])}

${rule('What is doing the work is usually <b>saturation, brightness and contrast</b>, not hue. A desaturated dim blue and a saturated bright blue are the same hue and completely different experiences.')}

${bites([
  '<b>Quoting a hue-to-emotion table in a design meeting.</b> It is the astrology of this trade. The defensible claims are about brightness, saturation, contrast and colour temperature, and they are enough.',
  '<b>Ignoring the room the audience is actually in.</b> Colour associations are cultural, and a touring show plays to several cultures. What reads as celebratory at home may not travel.',
  '<b>Treating adaptation as fixed.</b> An audience settles to whatever illuminant dominates within minutes. The fourth cue in a warm sequence does not look the way the first one did, and the plot was probably designed looking at each cue cold.',
  '<b>Saturated red and blue at the same depth.</b> Chromostereopsis makes one appear to float in front of the other. Beautiful on purpose, and an unpleasant surprise otherwise.',
])}
