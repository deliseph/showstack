...
${S('Private and public', 'Why a show network lives in numbers nobody can route to',
  ['Some address ranges are reserved and will never appear on the public internet. Routers along the way <em>drop</em> them, deliberately. Those are the private ranges, set aside by RFC 1918, and there are three: <span class="mono">10.0.0.0/8</span> with sixteen million addresses, <span class="mono">172.16.0.0/12</span> with a million, and <span class="mono">192.168.0.0/16</span> with sixty-five thousand.',
   'A show network lives in one of them, and the reason is not tidiness. A private address <strong>cannot be reached from outside</strong> without something deliberately forwarding to it. That is a security property you get for free by choosing the right numbers, and it is why the first question about any device that turns out to be reachable from the internet is what address range it was on.',
   'Which range to pick is mostly about collisions. <span class="mono">192.168.x.x</span> is the default on nearly every consumer router, so it is the range most likely to clash when somebody brings a laptop, a travel router or a home habit onto site &mdash; two networks that both think they are 192.168.1.0/24 cannot be bridged without renumbering one of them. <span class="mono">10.x.x.x</span> has room for VLANs and a sensible numbering scheme, which is why most permanent installations use it. <span class="mono">172.16</span> to <span class="mono">172.31</span> is the least used, and that is occasionally a reason to choose it.',
   'Everything outside those ranges is <strong>public</strong>, which means somebody owns it. Assigning yourself a public address on a private network works perfectly until the day something on your network needs to reach the real owner of that address, at which point the traffic goes to your own kit instead and the fault makes no sense at all.'])}
...
${S('The addresses that are a diagnosis', 'What 169.254 and 127.0.0.1 are telling you',
  ['Several reserved ranges are not configuration at all &mdash; they are the network reporting a state, and reading them saves a great deal of time.',
   ...
   '<strong>127.x.x.x</strong> is loopback: the machine talking to itself, never leaving the network card. A service bound to 127.0.0.1 is unreachable from anywhere else however good the cable is, and a great deal of "it works on my machine" is exactly this.',
   '<strong>224.0.0.0/4</strong> is multicast &mdash; a group rather than a host. Nothing is assigned one; devices subscribe. sACN sits at <span class="mono">239.255.x.x</span>, where the last two octets are the universe number, so <span class="mono">239.255.0.1</span> is universe 1 and you can read a capture without a lookup table.',
   'And <strong>100.64.0.0/10</strong> is carrier-grade NAT: your internet provider sharing one public address across many customers. It behaves like a private range, it is not yours to use, and it is why some remote-access setups that work at home fail on a venue connection.'])}

${S('At the terminal', 'The commands, and what in the output actually answers the question',
  ['Nearly every network question on a show has a one-line answer from a terminal. The reason people do not use them is that the command has a different name on every platform and the output arrives unlabelled, so the useful part is not the command but knowing what to read.',
   'Two are worth learning before the others. The <strong>route table</strong> &mdash; <span class="mono">route print</span>, <span class="mono">netstat -rn</span>, <span class="mono">ip route</span> &mdash; because a laptop connected to both the show network and wifi has two default routes, and traffic disappearing into the wrong interface is the single most common self-inflicted fault on a production network. And the <strong>multicast joins</strong> &mdash; <span class="mono">netstat -g</span>, <span class="mono">ip maddr show</span> &mdash; because a receiver that has not joined the group will never see the traffic however correctly it is being sent.',
   'The distinction worth carrying from <span class="mono">ping</span> is between its two failures. &ldquo;Destination host unreachable&rdquo; is <em>your</em> machine giving up before anything left; a timeout means the packet left and nothing came back. Those are different faults in different places, and treating them as the same thing is how people end up replacing a working cable.',
   'And from <span class="mono">tracert</span>: a hop showing stars has declined to answer, which is normal and not a fault. What matters is a hop where the time <em>jumps</em>, because that is the link doing the damage. <a href="/tools/#netcmd">The command reference on the tools page</a> has all of them side by side, with what to look for in each.'])}

${bites([
  '<b>Two default routes.</b> Wifi and a wired show network at once, and no way to know which one anything is using. Turn one off, or route deliberately.',
  '<b>192.168.1.0/24 everywhere.</b> The venue, the tour, the guest laptop and the travel router all defaulted to it. Two of them cannot be joined without renumbering.',
  '<b>Reading 169.254 as a working address.</b> It is a failure report. Something upstream did not hand out a lease.',
  '<b>Making up a public address.</b> It works until something needs to reach the real owner of it, and then the fault points nowhere useful.',
])}
