...
${S('First', 'Sampling, and the one rule about it', [
  ...
  'The rule is <b>Nyquist</b>: to represent a frequency you must sample at more than twice it. 44.1 kHz — the rate <a href="/standards/aes3/">AES3</a> and consumer digital audio were built around — gets you just past 22 kHz, which is past the top of human hearing, which is why that number exists. Sample below twice a frequency and it does not simply vanish — it comes back as a <em>different, lower</em> frequency that was never in the original. That is aliasing, it is unrecoverable once it has happened, and it is why every converter has an <b>anti-aliasing filter</b> in front of it removing anything above half the sample rate before it gets a chance.',
  'Two pieces of hardware sit behind that sentence and are worth naming, because they are the join between this page and the <a href="/learn/analogue/">analogue</a> one. The anti-aliasing filter is a real low-pass circuit, and a steep one at 44.1 kHz is hard to build without audible side effects near the top of the band &mdash; which is the actual engineering argument for higher sample rates, since sampling at 96 kHz lets the filter be gentle and lets it start well above anything you can hear. Behind it, a <b>sample-and-hold</b> stage catches the voltage and freezes it, because a converter cannot measure a value that is still moving while it is measuring it. Everything digital rests on a capacitor holding a charge steady for a few microseconds.',
